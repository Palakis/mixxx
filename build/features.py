...
class LiveBroadcasting(Feature):
    ...
    def configure(self, build, conf):
        ...
        if build.platform_is_linux:
            ...
            if not conf.CheckForPKG('shout-idjc', '2.4.4'):
                self.INTERNAL_LINK = True

        if not self.INTERNAL_LINK:
            self.INTERNAL_LINK = not conf.CheckLib(['libshout-idjc', 'shout-idjc'])

        if self.INTERNAL_LINK:
            print("Using internal shout_mixxx from lib/libshout-idjc")
